...
message_bits=[1,1,1,0]
memory = np.array([2])
g_matrix = np.array([[5, 7]]) # G(D) = [1+D^2, 1+D+D^2]
trellis = cc.Trellis(memory, g_matrix)
print(len(message_bits))
coded_bits=conv_encode(message_bits, trellis, termination = 'cont', puncture_matrix=None)
print('coded=',coded_bits)
print(len(coded_bits))

decoded=viterbi_decode(coded_bits, trellis, tb_depth=None, decoding_type='unquantized')
